...
export default async function AdminRequestsPage({
 ...
 <h1 className="text-2xl font-bold text-adm-txt">Pedidos de viagem</h1>
 <p className="mt-1 text-adm-muted">
 Todos os pedidos submetidos através do formulário online.
 </p>
 </div>
 ...
 <StatTile key={s} label={STATUS_LABELS[s]} value={stats[s]} />
 ))}
 </div>
...
"rounded-full border px-3.5 py-1.5 text-xs font-medium transition-colors",
 ...
 name="q"
 defaultValue={search}
 placeholder="Referência, nome, email, rota…"
 ...
 {/* List */}
 {requests.length === 0 ? (
 <EmptyState hasFilters={status !=="todos" || Boolean(search)} />
 ) : (
 ...
 <th className="px-4 py-3 font-semibold">Trajeto</th>
 ...
 {!r.team_notified && (
 <span
 title="A notificação para a equipa não foi entregue"
 ...
 {CHANNEL_LABELS[r.lead?.source_channel ??""] ??
 r.lead?.source_channel}
 </span>
 </Link>
 </td>
...
function EmptyState({ hasFilters }: { hasFilters: boolean }) {
 ...
 {hasFilters ?"Nenhum pedido corresponde ao filtro" :"Ainda sem pedidos"}
 </p>
 <p className="mx-auto mt-2 max-w-sm text-sm text-adm-muted">
 {hasFilters ? (
 <>
 Experimente limpar a pesquisa ou escolher outro estado.{""}
 <Link href="/admin/pedidos" className="font-medium text-adm-ember">
 Ver todos
 </Link>
 </>
 ) : (
"Os pedidos submetidos no formulário online aparecem aqui automaticamente."
 )}
 </p>
 </div>
 )
