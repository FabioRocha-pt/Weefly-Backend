...
function StageChip({
  ...
  const chip = t("admin.linkChip" + stage)
  const name = t("linkStages." + stage)
  ...
  if (locked) {
    return (
      <span
        title={t("admin.linkLockedTitle", { name })}
  ...
  return (
    <button
      type="button"
      onClick={copy}
      title={`${t("admin.linkCopyTitle", { name })}${submitted ? t("admin.linkCopyFilled") : ""}`}
      className={cn(
